...
def save_results(model_name, mode, dataset, test_auc, test_ap, test_accuracy, test_loss, bias=False):
    if bias:
    ...
    result_df = result_df.append({'dataset': dataset, 'mode' : mode, 'test_auc': test_auc, 'test_ap': test_ap,
                                  'test_accuracy' : test_accuracy, 'test_loss' : test_loss}, ignore_index=True)
...
class Runner(object):
    def __init__(self):
        ...
        self.start_train = 0
        # self.train_shots = [list(range(self.evalLength[i], self.len[i] - self.testlength[i] - self.evalLength[i])) for i in range(self.num_datasets)] #Changed
        ...
        # self.test_shots = [list(range(self.len[i] - self.testlength[i], self.len[i])) + list(range(0, self.evalLength[i])) for i in range(self.num_datasets)]
        # print(self.len[0])
        # print(self.train_shots[0])
        # print(self.val_shots[0])
        # print(self.test_shots[0])
    ...
    def tgclassification_test(self, readout_scheme, dataset_idx):
        ...
        tg_preds_binary = [1 if pred >= 0.5 else 0 for pred in tg_preds] #@NOTE use
    ...
    def run(self):
        ...
        # self.tgc_decoder.eval()
        # for dataset_idx in range(self.num_datasets):
        data_name = args.data_name[args.dataset[0]] if args.dataset[0] in args.data_name else args.dataset[0]
        self.model.train()
        ...
        for epoch in range(5):
            print("epoch:", epoch)
            for t_train_idx, t_train in enumerate(self.train_shots[0]):
                self.optimizer.zero_grad()

                edge_index = prepare(data[0], t_train)

                embeddings = self.model(edge_index, self.x)

                # graph readout
                tg_readout = readout_function(embeddings, self.readout_scheme)
                tg_embedding = torch.cat((tg_readout, torch.from_numpy(self.t_graph_feat[0][t_train_idx]).to(args.device)))

                # graph classification
                tg_label = self.t_graph_labels[0][t_train_idx].float().view(1, )
                tg_pred = self.tgc_decoder(tg_embedding.view(1, tg_embedding.size()[0]).float()).sigmoid()

                tg_labels.append(tg_label.cpu().numpy())
                tg_preds.append(tg_pred.cpu().detach().numpy())
                t_loss = self.criterion(tg_pred, tg_label)
                t_loss.backward()
                self.optimizer.step()
            ...
            print("train_loss: ", np.mean(epoch_losses))
            for t_val in self.val_shots[0]:
                with torch.no_grad():
                    edge_index = prepare(data[0], t_val)
                    embeddings = self.model(edge_index, self.x)
                    self.model.update_hiddens_all_with(embeddings)

            
            test_auc, test_ap, test_accuracy, test_loss = self.tgclassification_test(self.readout_scheme, 0)
...
if __name__ == '__main__':
    ...
    t_graph_labels = []
    t_graph_feat = []
    args.dataset, data = load_multiple_datasets("dataset_package_social.txt")
    for dataset in args.dataset:
    ...
    category = "HTGN" #"no_shuffle" #"no_mem_update" #"HTGN"#"no_init" #"HTGN"
    # for data_num in [2, 3]:
    
    
    for n_data in [64]:
        for seed in [710, 720, 800]:
            # for ratio in [0.0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.5, 0.55, 0.6]:
                # args.test_ratio = ratio
                set_random(seed)
