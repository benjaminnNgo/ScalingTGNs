...
def save_epoch_results(epoch, test_auc, test_ap, time, dataset=None):
    if dataset is None:
        result_path = "../data/output/epoch_result/average/{}_{}_{}_epochResult.csv".format(len(args.dataset),
        ...
        result_path = result_folder + "/{}_{}_{}_epochResult.csv".format(dataset,
                                                                            len(args.dataset),
                                                                            args.model, args.seed,
                                                                            args.curr_time)
    ...
    result_df = result_df._append({'epoch': int(epoch), 'test_auc': test_auc, 'test_ap': test_ap, 'time': time},
                                  ignore_index=True)
...
def save_epoch_training(epoch, train_auc, train_ap, loss, time, dataset=None):
    if dataset is None:
        result_path = "../data/output/training_test/average/{}_{}_{}_epochResult.csv".format(len(args.dataset),
                                                                                                args.model, args.seed,
                                                                                                )
    else:
        # data_name = args.data_name[args.dataset[dataset]] if args.dataset[dataset] in args.data_name else args.dataset[dataset]
    ...
    result_df = result_df._append(
        {'epoch': int(epoch), 'train_loss': loss, 'train_auc': train_auc, 'train_ap': train_ap, 'time': time},
        ignore_index=True)
    result_df.to_csv(result_path, index=False)


def save_epoch_results_test(epoch, test_auc, test_ap):
    result_path = "../data/output/epoch_result/average/{}_{}_{}_{}_testResult.csv".format(len(args.dataset), args.model,
                                                                                          args.seed, args.curr_time)
    if not os.path.exists(result_path):
...
def save_epoch_results_eval(epoch, test_auc, test_ap):
    result_path = "../data/output/bias_test/{}_{}_{}_{}_biasResult.csv".format(len(args.dataset), args.model, args.seed,
                                                                               args.curr_time)
    if not os.path.exists(result_path):
        result_df = pd.DataFrame(columns=["epoch", "test_auc", "test_ap"])
    else:
...
class Runner(object):
    def __init__(self):
        ...
        self.start_train = 0
        self.train_shots = [list(range(0, self.len[i] - self.testlength[i] - self.evalLength[i])) for i in
                            range(self.num_datasets)]  # Changed
        self.eval_shots = [
            list(range(self.len[i] - self.testlength[i] - self.evalLength[i], self.len[i] - self.testlength[i])) for i
            in range(self.num_datasets)]  # Changed
        ...
        self.model_path = '{}/saved_models/fm/{}_seed_{}_{}.pth'.format(model_file_path,
                                                                           args.model,
                                                                           args.seed,
                                                                           self.num_datasets,
                                                                           )

        self.mlp_path = '{}/saved_models/fm/{}_seed_{}_{}_mlp.pth'.format(model_file_path,
                                                                             args.model,
                                                                             args.seed,
                                                                             self.num_datasets,
                                                                             )

        self.model_chkp = '{}/saved_models/fm/checkpoint/{}_seed_{}_{}.pth'.format(model_file_path,
                                                                                      args.model,
                                                                                      args.seed,
                                                                                      self.num_datasets,
                                                                                      )
    ...
    def load_feature(self):
        if args.trainable_feat:
            ...
            if args.pre_defined_feature is not None:
                ...
                # print("=========={}".format(args.num_nodes))
                # print("=========={}".format(args.max_node_id))
        ...
        for t_eval_idx, t in enumerate(self.eval_shots[dataset_idx]):
            ...
            with torch.no_grad():
                ...
                tg_embedding = torch.cat((tg_readout,
                                          torch.from_numpy(self.t_graph_feat[dataset_idx][
                                                               t_eval_idx + len(self.train_shots[dataset_idx])]).to(
                                              args.device)))

                # graph classification
                tg_labels.append(
                    self.t_graph_labels[dataset_idx][t_eval_idx + len(self.train_shots[dataset_idx])].cpu().numpy())
                tg_preds.append(
    ...
    def tgclassification_test(self, epoch, readout_scheme, dataset_idx):
        ...
        for t_test_idx, t in enumerate(self.test_shots[dataset_idx]):
            ...
            with torch.no_grad():
                ...
                tg_embedding = torch.cat((tg_readout,
                                          torch.from_numpy(self.t_graph_feat[dataset_idx][
                                                               t_test_idx + len(self.train_shots[dataset_idx])]).to(
                                              args.device)))
    ...
    def test_bias(self):
        for dataset_idx in range(self.num_datasets):
            ...
            for t_train_idx, t_train in enumerate(self.train_shots[dataset_idx]):
                ...
                edge_index, pos_index, neg_index, activate_nodes, edge_weight, _, _ = prepare(data[dataset_idx],
                                                                                              t_train)
                ...
                tg_embedding = torch.cat((tg_readout,
                                          torch.from_numpy(self.t_graph_feat[dataset_idx][t_train_idx]).to(
                                              args.device)))
    ...
    def run(self):
        ...
        # for epoch in range(1, 2):
            ...
            for dataset_idx in dataset_rnd:
                ...
                tg_labels, tg_preds = [], []
                data_name = args.data_name[args.dataset[dataset_idx]] if args.dataset[
                                                                             dataset_idx] in args.data_name else \
                args.dataset[dataset_idx]
            ...
            # Validating
            if best_eval_auc < np.mean(
                    eval_aucs) or epoch < args.min_epoch:  # Use AUC as metric to define early stoping
                patience = 0
            ...
            if epoch == 1 or epoch % args.log_interval == 0:
                ...
                logger.info("Epoch:{}, Loss: {:.4f}, Time: {:.3f}, GPU: {:.1f}MiB".format(epoch, avg_epoch_loss,
                                                                                          ...
                                                                                          gpu_mem_alloc))
                logger.info(
                    "Train: Epoch:{}, AUC: {:.4f}, AP: {:.4f}, Loss: {:.4f}".format(epoch, np.mean(train_aucs),
                                                                                    np.mean(train_aps), avg_epoch_loss))
                logger.info(
        ...
        log_path = '../data/output/log/{}_{}_seed_{}.txt'.format(args.model,
                                                                       args.seed,
                                                                       self.num_datasets
                                                                       )
...
if __name__ == '__main__':
    ...
    from script.utils.data_util import loader, prepare_dir, load_multiple_datasets, find_max_node_id_package
    ...
    args.dataset, data = load_multiple_datasets("node_id_package.txt")

    args.max_node_id = find_max_node_id_package("node_id_package.txt")
    ...
    init_logger(
        '../data/output/log/{}_{}_seed_{}_log.txt'.format(len(args.dataset), args.model, args.seed))
