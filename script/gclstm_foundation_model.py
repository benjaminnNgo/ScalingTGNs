...
def extra_dataset_attributes_loading(args, readout_scheme='mean', task="edge_gs"):
    """
...
def save_epoch_results(epoch, test_auc, test_ap, time, dataset=None):
    if dataset is None:
        result_folder = "{}/epoch_result/average/".format(model_file_path)
        ...
        # print(data_name)
        result_folder = "{}/epoch_result/data/{}".format(model_file_path, dataset)
        result_path = result_folder + "/{}_epochResult.csv".format(filename)
    ...
    if not os.path.exists(result_path):
        ...
        result_df = pd.read_csv(result_path)
    result_df = result_df.append({'epoch': int(epoch), 'test_auc': test_auc, 'test_ap': test_ap, 'time': time},
                                 ignore_index=True)
    result_df.to_csv(result_path, index=False)
...
def save_epoch_training(epoch, train_auc, train_ap, loss, time, dataset=None):
    if dataset is None:
        ...
        result_folder = "{}/training_test/average/".format(model_file_path)
        result_path = result_folder + "{}_epochResult.csv".format( filename)
    else:


        result_folder = "{}/training_test/data/{}".format(model_file_path, dataset)
        result_path = result_folder + "/{}_epochResult.csv".format(filename)
...
class Runner():
    def __init__(self):
        ...
        init_logger('{}/output/log/{}_{}_seed_{}_{}_log.txt'.format(DATA_PATH, args.model, args.pack, args.seed,
                                                                    args.nhid))
        logger.info("INFO: Args: {}".format(args))
        self.t_graph_labels, self.t_graph_feat = extra_dataset_attributes_loading(args, task=task)
        ...
        self.testlength = [math.floor(self.len[i] * args.test_ratio) for i in
                           range(self.num_datasets)]  # Re-calculate number of test snapshots
        self.evalLength = [math.floor(self.len[i] * args.val_ratio) for i in range(self.num_datasets)]

        self.train_shots_mask = [list(range(0, self.len[i] - self.testlength[i] - self.evalLength[i])) for i in
                                 range(self.num_datasets)]  # Changed
        self.eval_shots_mask = [
        ...
        self.test_shots_mask = [list(range(self.len[i] - self.testlength[i], self.len[i])) for i in
        ...
        for epoch in range(1, args.max_epoch + 1):
            ...
            for dataset_idx in dataset_rnd:
                ...
                for snapshot_idx in self.train_shots_mask[dataset_idx]:
                    ...
                    tg_embedding = torch.cat(
                        (tg_readout, torch.from_numpy(self.t_graph_feat[dataset_idx][snapshot_idx]).to(args.device)))
                    ...
                    self.detach_hidden()
                    print(f"SNAPSHOT : {snapshot_idx} , Loss : {t_loss}")
...
if __name__ == '__main__':
    ...
    datasets_package_path = "social_dataset_package_{}.txt".format(args.pack)
    filename = "{}_{}_seed_{}".format(args.model, args.pack, args.seed)
    init_logger(
        '../../data/output/log/{}_{}_seed_{}_log.txt'.format(args.model, args.pack, args.seed))
